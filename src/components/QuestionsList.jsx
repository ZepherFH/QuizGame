...
const QuestionList = ({
    ...
    const { id, question, answer1, answer2, answer3, answerTrue } = Question;

    const deleteListQuestion = () => {
        const newList = listQuestions.filter(Question => Question.id !== id)
    ...
    const editListQuestion = async () => {
        const { value } = await Swal.fire({
            title: "Informacion de la Question",
            html: `
                <input 
                    type="text"
                    id="question" 
        ...
        const newList= listQuestions.map((Question) => {
            if(Question.id === id){
                Question.question = value.question;
                Question.answer1 = value.answer1;
                Question.answer2 = value.answer2;
                Question.answer3 = value.answer3;
                Question.answerTrue = trueAnswer;
            }
            return Question;
        })
